...
void MAX17201::set_temperature_alerts(int8_t max_temperature_threshold,
        int8_t min_temperature_threshold)
{
    if (_i2cAddress != I2CAddress::ModelGaugeM5Address) {
        _i2cAddress = I2CAddress::ModelGaugeM5Address;
    }

    uint16_t data = ((max_temperature_threshold & 0xFF) << 8) | (min_temperature_threshold &
                    0xFF); // set threshold
...
{
    uint16_t data;
    data = ((static_cast<uint16_t>(VEmpty * 100) & 0x1FF) << 7)
            | (static_cast<uint8_t>((VEmpty + 0.5) * 25) & 0x7F);
